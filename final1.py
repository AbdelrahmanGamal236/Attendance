...
frame_resizing = 0.25
###time ##
def check_subject_time():
    today = datetime.now()
    day, month, year = today.day, today.month, today.year

    arabic = datetime(year, month, day, 9) #the number serve the hours 
    english = datetime(year, month, day, 11)
    math = datetime(year, month, day, 13)

    current_time = datetime.now()

    if current_time >= arabic and current_time < english:  #the period of class 
        return 'Arabic'
    elif current_time >= english and current_time < math:
        return 'English'
    elif current_time >= math:
        return'Math'
    ##camera ##
    ...
    while runing:
        ...
        for encodeFace,faceLoc in zip(encodesCurFrame,facesCurFrame):
            ...
            if matches[matchIndex]:
                ...
                subject_name = check_subject_time()
                make_attendance(name, subject_name)
...
# email_address = "projectt959@gmail"
# email_password = "Team$12345"
# smtp_server = "smtp.gmail.com"
# smtp_port = 587  # For example, use 587 for TLS
...
#     if matches[matchIndex]:
#         user = classNames[matchIndex].upper()
#         print(user)
#         # ... Your existing code ...

#         # Send email notification to parents or guardians
#         subject = "Attendance Report for " + user
#         body = "Your child " + user + " attended the class at " + datetime.now().strftime("%H:%M:%S")
#         to_email = "kareemgoda48@gmail.com "  # Replace with actual email address
...
if __name__ == "__main__":
    ...
    start_btn = st.button("Start Attendance", key='start', on_click=main)
    end_btn = st.button("End Attendance", key='end', on_click=end)
